...
OUTPUT_DIR = "./llama-70b-multilingual-refined"
...
def formatting_prompts_func(example):
    ...
    for i in range(len(example['source'])):
        ...
        sys_msg = f"You are a professional {lang} editor. Fix the draft based on back-translation."
        
        text = (
            f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n{sys_msg}<|eot_id|>"
            f"<|start_header_id|>user<|end_header_id|>\n\n"
            f"Source: {example['source'][i]}\n"
            f"Draft: {example['draft'][i]}\n"
            f"Back-trans: {example['back_trans'][i]}\n\n"
